...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  private readonly routes = new Set(['/dashboard', '/alerts', '/assignments', '/history', '/reports',
    '/admin/users', '/admin/profiles', '/admin/menu-options']);
  ...
  private menuItem(option: MenuOption, depth = 0): NavItem | null {
    ...
    return { label: option.name, icon: 'folder', route, children: children.length ? children : undefined,
      expanded: true, disabled: !route || !this.routes.has(route) };
  }

  ngOnInit(): void {
    this.subscription = merge(of(undefined), this.users.menuChanges).pipe(
      switchMap(() => this.users.getMenuOptions(false, true).pipe(catchError(() => of(null))))
    ).subscribe(data => {
      // Las pantallas operativas existentes siempre deben ser accesibles.
      this.navSections = this.defaultSections
        .filter(section => this.auth.isAdmin() || section.title !== 'ADMINISTRACIÓN')
        .map(section => ({ ...section, items: section.items.map(item => ({ ...item })) }));
      if (!data) return;
      const items = data.menu_options.map(option => this.menuItem(option))
        .filter((item): item is NavItem => item !== null);
      if (items.length) this.navSections.push({ title: 'MENÚ CONFIGURADO', items });
    });
  }
