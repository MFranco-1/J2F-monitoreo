...
def get_available_technicians():
    technicians = [u for u in User.query.order_by(User.full_name, User.id).all() if is_technician(u)]
    return jsonify({"technicians": [{
        "id": user.id,
        "full_name": user.full_name,
        "active_assignments_count": user.active_assignments_count,
    } for user in technicians]}), 200
...
def auto_assign(data, current_user_id):
    ...
    if not technicians:
        return jsonify({"error": "No hay Técnicos activos disponibles"}), 409
    selected = min(technicians, key=lambda u: (u.active_assignments_count, u.id))
    ...
    db.session.add(assignment)
    _log(alert.id, current_user_id, "assigned", f"Auto-asignada al técnico: {selected.full_name}")
    db.session.commit()
    return jsonify({"message": f"Alerta #{alert.id} asignada a {selected.full_name}",
                    "assignment": assignment.to_dict(),
                    "technician": selected.to_dict(include_profile=False)}), 201
...
def _transition_to_in_progress(alert, user_id):
    if alert.state.name == "Abierto":
        ...
        result = _change_state(alert, user_id, "En Progreso", "Asignación de técnico")
